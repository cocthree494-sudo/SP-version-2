...
- [x] **T-079 — Add a searchable product documentation center**
...
- [ ] **T-082 — Prove tenant-isolated bots, manual knowledge, and Gemini generation**
  Depends on: T-023, T-035, T-036, T-043, T-046, T-052, T-055, T-060, T-072, T-074
  Keep this acceptance deliberately small. Deploy the current main branch to the VPS,
  create two separate tenant accounts, and create a bot for each through the real
  dashboard. Prove PostgreSQL persistence across refresh and sign-in, including that
  identical bot names do not collide. Account A must receive `403` or `404` when it
  attempts to list, read, edit, or delete Account B's bot by identifier, and vice versa.

  Add one Manual Q&A knowledge source to each bot with distinct, non-guessable marker
  facts. Wait for background ingestion to reach `ready`, then prove each bot retrieves
  and cites only its own fact and does not answer from the other tenant's source. Verify
  tenant and bot scoping for the source, job, document, chunks, conversation, messages,
  cache/retrieval path, and usage records. Any cross-tenant leakage or ambiguous ownership
  is release-blocking.

  Keep Google/Gemini enabled through its approved OpenAI-compatible endpoint and
  maintained model catalog. Add a Gemini API key to Account A only through the
  authenticated write-only provider form, verify masked storage, and select
  `gemini-2.5-flash` with `tenant_only` routing. Prove a grounded answer is generated by
  that Gemini target and that usage/audit evidence records the expected provider and
  model without exposing the secret. Account B must not list, access, or borrow Account
  A's credential; missing, invalid, or revoked tenant-only credentials must fail safely
  without a hidden platform or cross-tenant fallback.

  Run only lightweight focused checks locally. Run PostgreSQL/Redis integration,
  API/worker, production build/deploy, and focused desktop browser acceptance on the VPS.
  Website crawling, broad provider-catalog acceptance, external channels, and the
  platform-admin dashboard are explicitly outside this narrowed task.
