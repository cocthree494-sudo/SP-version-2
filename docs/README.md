# Documentation

Additional documentation beyond the root `CONTEXT.md`, `PLAN.md`, `TASKS.md`, and `FEATURES.md` goes here.

## Contents

- [Tenancy and isolation](tenancy.md)
- [Authentication](authentication.md)
- [Bots and widget credentials](bots.md)
- [Usage accounting](usage.md)
- [Storage and ingestion foundations](ingestion.md)
- [Knowledge ingestion and retrieval](knowledge.md)
- [Provider-neutral AI adapters](providers.md)
- [Conversations, context, and retention](conversations.md)
- [Grounded support-agent orchestration](agent-orchestration.md)
- [Anonymous widget chat API](widget-chat-api.md)
- [Agent quality and safety evaluations](agent-evaluations.md)
- [Dashboard authentication and shell](dashboard-auth.md)
- [Dashboard bot and knowledge management](dashboard-management.md)
- [Widget implementation and installation](widget.md)
- [Critical-path end-to-end coverage](e2e.md)
- [Production operations](operations.md)
- [MVP acceptance review](mvp-acceptance.md)
