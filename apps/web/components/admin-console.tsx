...
function DataRow({ section, item, onAction }: { section: Section; item: TenantRow | UserRow | UsageRow | IngestionRow | AuditRow; onAction: (value: { kind: "tenant" | "user" | "sessions"; id: string; desired?: string }) => void }) {
  ...
  if (section === "ingestion") { const row = item as IngestionRow; return <tr><td><strong>{row.tenant_name}</strong><small>{row.source_name || "Unknown source"}</small></td><td><strong>{row.job_type}</strong><small>{row.attempts} / {row.max_attempts} attempts</small></td><td><StatusBadge value={row.state} /></td><td><div className="admin-progress"><i style={{ width: `${row.progress_percent}%` }} /></div><small>{row.progress_percent}%</small></td><td>{row.error_code ? <span className="admin-failure" title={row.error_message || row.error_code}>{row.error_code}</span> : "-"}</td><td>{formatDate(row.created_at)}</td></tr>; }
